...
def create_encoder_for_edit_distance(input_shape, embedding_dim=64):
    ...
    # 順序情報を保持するLSTM
    x = layers.LSTM(embedding_dim, return_sequences=False)(x)
    x = layers.Dropout(0.5)(x)
    ...
    return keras.Model(inputs=inputs, outputs=x, name="edit_distance_encoder")

def create_branchB_encoder(seq_len, primes=[2,3,5,7], max_vp=40, emb_dim=8):
    """
    ブランチB: (mod p, v_p(x)) を Embedding して時系列処理するエンコーダ
    入力: shape = (seq_len, len(primes)*2)  (整数型)
    出力: 埋め込みベクトル
    """
    input_B = keras.Input(shape=(seq_len, len(primes)*2), dtype="int32", name="branchB_input")

    embedded_features = []

    for i in range(0, len(primes)*2, 2):
        # --- mod p 部分 ---
        mod_input = input_B[:, :, i]  # shape (batch, seq_len)
        mod_emb = layers.Embedding(input_dim=primes[int(i/2)], output_dim=emb_dim, name=f"mod{primes[int(i/2)]}_emb")(mod_input)

        # --- v_p 部分 ---
        vp_input = input_B[:, :, i + 1]
        vp_emb = layers.Embedding(input_dim=max_vp+1, output_dim=emb_dim, name=f"vp{primes[int(i/2)]}_emb")(vp_input)

        # 結合して一つの特徴チャネルにする
        feat_emb = layers.Concatenate(axis=-1)([mod_emb, vp_emb])  # (batch, seq_len, 2*emb_dim)
        embedded_features.append(feat_emb)

    # 全primeの特徴を結合
    x = layers.Concatenate(axis=-1)(embedded_features)  # (batch, seq_len, len(primes)*2*emb_dim)
    ...
    return keras.Model(inputs=input_B, outputs=x, name="branchB_encoder")

def create_encoder_for_edit_distance_with_bins(bins_input_shape, n_bins, embedding_dim=64):
    """分位ビニングを追加したエンコーダー"""
    ...
    # 編集距離計算に有効な特徴量を抽出(ビンデータ分の情報が増えるため次元数を上げた方が良い？この場合過学習対策推奨)
    x = layers.Dense(embedding_dim, activation='relu')(x_bins)
    ...
    return keras.Model(inputs=input_bins, outputs=x, name="edit_distance_encoder_with_bins")

def create_comparison_classifier_model(input_shape, input_B_shape, input_bin_shape, seq_len, primes=[2,3,5,7], max_vp=40, embedding_dim=EMBEDDING_DIM):
    # エンコーダは共通で利用
    encoderA = create_encoder_for_edit_distance(
        input_shape, 
        embedding_dim)
    
    encoderB = create_branchB_encoder(
        seq_len,
        primes,
        max_vp
    )

    encoder_bin = create_encoder_for_edit_distance_with_bins(
        input_bin_shape,
        N_BINS,
        embedding_dim
    )
    ...
    # 3組の入力: 数列A, 数列B, 数列C
    inputA_A = keras.Input(shape=input_shape, name="input_A_A")
    inputB_A = keras.Input(shape=input_shape, name="input_B_A")
    inputC_A = keras.Input(shape=input_shape, name="input_C_A")

    inputA_B = keras.Input(shape=input_B_shape, dtype="int32", name="input_A_B")
    inputB_B = keras.Input(shape=input_B_shape, dtype="int32", name="input_B_B")
    inputC_B = keras.Input(shape=input_B_shape, dtype="int32", name="input_C_B")

    inputA_bin = keras.Input(shape=input_bin_shape, dtype="int32", name="input_A_bin")
    inputB_bin = keras.Input(shape=input_bin_shape, dtype="int32", name="input_B_bin")
    inputC_bin = keras.Input(shape=input_bin_shape, dtype="int32", name="input_C_bin")

    # 各数列を埋め込みベクトルに変換
    embeddingA_A = encoderA(inputA_A)
    embeddingB_A = encoderA(inputB_A)
    embeddingC_A = encoderA(inputC_A)

    embeddingA_B = encoderB(inputA_B)
    embeddingB_B = encoderB(inputB_B)
    embeddingC_B = encoderB(inputC_B)

    embeddingA_bin = encoder_bin(inputA_bin)
    embeddingB_bin = encoder_bin(inputB_bin)
    embeddingC_bin = encoder_bin(inputC_bin)

    embedding_A = layers.Concatenate()([embeddingA_A, embeddingA_B, embeddingA_bin])
    embedding_B = layers.Concatenate()([embeddingB_A, embeddingB_B, embeddingB_bin])
    embedding_C = layers.Concatenate()([embeddingC_A, embeddingC_B, embeddingC_bin])
    ...
    # モデルの構築
    model = keras.Model(
        inputs={"input_A_A":inputA_A, "input_A_B":inputA_B, "input_A_bin":inputA_bin,"input_B_A":inputB_A, "input_B_B":inputB_B, "input_B_bin":inputB_bin, "input_C_A":inputC_A, "input_C_B":inputC_B, "input_C_bin":inputC_bin}, 
...
if __name__ == "__main__":
    ...
    # NumPy変換と数列の変換
    X_A_A = np.array([pad_and_stack_features(seq) for seq in X_A_raw], dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH, INPUT_NUM)
    X_B_A = np.array([pad_and_stack_features(seq) for seq in X_B_raw], dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH, INPUT_NUM)
    X_C_A = np.array([pad_and_stack_features(seq) for seq in X_C_raw], dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH, INPUT_NUM)
    X_A_B = np.array([transform_sequence_mod_vp(seq, PRIMES) for seq in X_A_raw], dtype=np.int32)
    X_B_B = np.array([transform_sequence_mod_vp(seq, PRIMES) for seq in X_B_raw], dtype=np.int32)
    X_C_B = np.array([transform_sequence_mod_vp(seq, PRIMES) for seq in X_C_raw], dtype=np.int32)
    X_A_bin = np.array(transform_sequence_diff(X_A_raw), dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH-1)
    X_B_bin = np.array(transform_sequence_diff(X_B_raw), dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH-1)
    X_C_bin = np.array(transform_sequence_diff(X_C_raw), dtype=np.float64).reshape(-1, MAX_SEQUENCE_LENGTH-1)
    y_labels = np.array(y_labels_raw, dtype=np.int32)

    # 訓練データとテストデータに分割
    # 3つの入力 (X_A_A, X_B_A, X_C_A) と1つのターゲット (y_labels)
    X_train_A_A, X_test_A_A, X_train_A_B, X_test_A_B, X_train_A_bin, X_test_A_bin, X_train_B_A, X_test_B_A, X_train_B_B, X_test_B_B, X_train_B_bin, X_test_B_bin, X_train_C_A, X_test_C_A, X_train_C_B, X_test_C_B, X_train_C_bin, X_test_C_bin, \
    y_train, y_test = train_test_split(
        X_A_A, X_A_B, X_A_bin, X_B_A, X_B_B, X_B_bin, X_C_A, X_C_B, X_C_bin, y_labels,
    ...
    # 訓練データを検証データと分割
    X_train_A_A, X_val_A_A, X_train_A_B, X_val_A_B, X_train_A_bin, X_val_A_bin, X_train_B_A, X_val_B_A, X_train_B_B, X_val_B_B, X_train_B_bin, X_val_B_bin, X_train_C_A, X_val_C_A, X_train_C_B, X_val_C_B, X_train_C_bin, X_val_C_bin, \
    y_train, y_val = train_test_split(
        X_train_A_A, X_train_A_B, X_train_A_bin, X_train_B_A, X_train_B_B, X_train_B_bin, X_train_C_A, X_train_C_B, X_train_C_bin, y_train,
        test_size=1 - TRAIN_TEST_SPLIT_RATIO, 
        random_state=42,
    ...
    scaler.fit(np.concatenate([X_train_B_A, X_train_C_A], axis=0).reshape(-1, F))

    # 決定したfitから各サンプルを標準化
    X_train_A_A = scaler.transform(X_train_A_A.reshape(-1, F)).reshape(-1, L, F)
    X_train_B_A = scaler.transform(X_train_B_A.reshape(-1, F)).reshape(-1, L, F)
    X_train_C_A = scaler.transform(X_train_C_A.reshape(-1, F)).reshape(-1, L, F)
    X_val_A_A = scaler.transform(X_val_A_A.reshape(-1, F)).reshape(-1, L, F)
    X_val_B_A = scaler.transform(X_val_B_A.reshape(-1, F)).reshape(-1, L, F)
    X_val_C_A = scaler.transform(X_val_C_A.reshape(-1, F)).reshape(-1, L, F)
    X_test_A_A = scaler.transform(X_test_A_A.reshape(-1, F)).reshape(-1, L, F)
    X_test_B_A = scaler.transform(X_test_B_A.reshape(-1, F)).reshape(-1, L, F)
    X_test_C_A = scaler.transform(X_test_C_A.reshape(-1, F)).reshape(-1, L, F)

    # モデルで扱いやすいfloat32型に変換
    X_train_A_A = X_train_A_A.astype(np.float32)
    X_train_B_A = X_train_B_A.astype(np.float32)
    X_train_C_A = X_train_C_A.astype(np.float32)
    X_val_A_A = X_val_A_A.astype(np.float32)
    X_val_B_A = X_val_B_A.astype(np.float32)
    X_val_C_A = X_val_C_A.astype(np.float32)
    X_test_A_A = X_test_A_A.astype(np.float32)
    X_test_B_A = X_test_B_A.astype(np.float32)
    X_test_C_A = X_test_C_A.astype(np.float32)
    ...
    # --- 5. モデルの作成と訓練 ---
    print(X_train_A_A[0].shape)
    print(X_train_A_B[0].shape)
    print(X_train_A_bin[0].shape)
    model = create_comparison_classifier_model(X_train_A_A[0].shape, X_train_A_B[0].shape, X_train_A_bin[0].shape, MAX_SEQUENCE_LENGTH, PRIMES, MAX_VP, EMBEDDING_DIM) # 新しいモデルを呼び出し
    
    print("\n--- Starting comparison model training ---")
    history = model.fit(
        {"input_A_A":X_train_A_A, "input_A_B":X_train_A_B, "input_A_bin":X_train_A_bin, "input_B_A":X_train_B_A, "input_B_B":X_train_B_B, "input_B_bin":X_train_B_bin, "input_C_A":X_train_C_A, "input_C_B":X_train_C_B, "input_C_bin":X_train_C_bin}, # 3つの入力
        y_train,
        epochs=EPOCHS,
        batch_size=BATCH_SIZE,
        validation_data=({"input_A_A":X_val_A_A, "input_A_B":X_val_A_B, "input_A_bin":X_val_A_bin, "input_B_A":X_val_B_A, "input_B_B":X_val_B_B, "input_B_bin":X_val_B_bin, "input_C_A":X_val_C_A, "input_C_B":X_val_C_B, "input_C_bin":X_val_C_bin}, y_val), # 訓練データの一部を検証データとして使用
        verbose=1 ,
    ...
    loss, accuracy, precision, recall, auc = model.evaluate({"input_A_A":X_test_A_A, "input_A_B":X_test_A_B, "input_A_bin":X_test_A_bin, "input_B_A":X_test_B_A, "input_B_B":X_test_B_B, "input_B_bin":X_test_B_bin, "input_C_A":X_test_C_A, "input_C_B":X_test_C_B, "input_C_bin":X_test_C_bin}, y_test, verbose=0)
    
    # 予測の実行 (混同行列やF1スコア計算のため)
    y_pred_prob = model.predict({"input_A_A":X_test_A_A, "input_A_B":X_test_A_B, "input_A_bin":X_test_A_bin, "input_B_A":X_test_B_A, "input_B_B":X_test_B_B, "input_B_bin":X_test_B_bin, "input_C_A":X_test_C_A, "input_C_B":X_test_C_B, "input_C_bin":X_test_C_bin}).flatten()
