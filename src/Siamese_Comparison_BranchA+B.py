...
def create_comparison_classifier_model(input_shape, input_B_shape, seq_len, primes=[2,3,5,7], max_vp=40, embedding_dim=EMBEDDING_DIM):
    # エンコーダは共通で利用
    ...
    embedding_A = layers.Concatenate()([embeddingA_A, embeddingA_B])
    embedding_B = layers.Concatenate()([embeddingB_A, embeddingB_B])
    embedding_C = layers.Concatenate()([embeddingC_A, embeddingC_B])
    ...
    # 選択肢4: 結合 + 比率 (ゼロ除算注意)
    # ratio_features = tf.math.divide_no_nan(features_AB, features_AC+1e-8)
    ...
    # モデルの構築
    model = keras.Model(
        inputs={"input_A_A":inputA_A, "input_A_B":inputA_B, "input_B_A":inputB_A, "input_B_B":inputB_B, "input_C_A":inputC_A, "input_C_B":inputC_B}, 
...
if __name__ == "__main__":
    ...
    X_train_A_A, X_test_A_A, X_train_A_B, X_test_A_B, X_train_B_A, X_test_B_A, X_train_B_B, X_test_B_B, X_train_C_A, X_test_C_A, X_train_C_B, X_test_C_B, \
    y_train, y_test = train_test_split(
        X_A_A, X_A_B, X_B_A, X_B_B, X_C_A, X_C_B, y_labels,
    ...
    # 訓練データを検証データと分割
    X_train_A_A, X_val_A_A, X_train_A_B, X_val_A_B, X_train_B_A, X_val_B_A, X_train_B_B, X_val_B_B, X_train_C_A, X_val_C_A, X_train_C_B, X_val_C_B, \
    y_train, y_val = train_test_split(
        X_train_A_A, X_train_A_B, X_train_B_A, X_train_B_B, X_train_C_A, X_train_C_B, y_train,
        test_size=1 - TRAIN_TEST_SPLIT_RATIO, 
        random_state=42,
    ...
    # --- 5. モデルの作成と訓練 ---
    model = create_comparison_classifier_model(X_train_A_A[0].shape, X_train_A_B[0].shape, MAX_SEQUENCE_LENGTH, PRIMES, MAX_VP, EMBEDDING_DIM) # 新しいモデルを呼び出し
    
    print("\n--- Starting comparison model training ---")
    history = model.fit(
        {"input_A_A":X_train_A_A, "input_A_B":X_train_A_B, "input_B_A":X_train_B_A, "input_B_B":X_train_B_B, "input_C_A":X_train_C_A, "input_C_B":X_train_C_B}, # 3つの入力
        y_train,
        epochs=EPOCHS,
        batch_size=BATCH_SIZE,
        validation_data=({"input_A_A":X_val_A_A, "input_A_B":X_val_A_B, "input_B_A":X_val_B_A, "input_B_B":X_val_B_B, "input_C_A":X_val_C_A, "input_C_B":X_val_C_B}, y_val), # 訓練データの一部を検証データとして使用
        verbose=1 ,
    ...
    loss, accuracy, precision, recall, auc = model.evaluate({"input_A_A":X_test_A_A, "input_A_B":X_test_A_B, "input_B_A":X_test_B_A, "input_B_B":X_test_B_B, "input_C_A":X_test_C_A, "input_C_B":X_test_C_B}, y_test, verbose=0)
    
    # 予測の実行 (混同行列やF1スコア計算のため)
    y_pred_prob = model.predict({"input_A_A":X_test_A_A, "input_A_B":X_test_A_B, "input_B_A":X_test_B_A, "input_B_B":X_test_B_B, "input_C_A":X_test_C_A, "input_C_B":X_test_C_B}).flatten()
