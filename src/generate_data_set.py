...
def generate_single_sample_first_sequence(depth:int, numeric_sequence_length=20)->dict:
    ...
    necessary_numeric_sequence_length_1 = generate_program.calculate_original_sequence_length(pr_1.root_node, numeric_sequence_length)
    ...
    return {
        ...
        'token_sequence_1': pr_1.get_token_sequence(),
...
def generate_single_sample_second_sequence(data:dict, depth:int, numeric_sequence_length=20)->dict:
    ...
    necessary_numeric_sequence_length_2 = generate_program.calculate_original_sequence_length(pr_2.root_node, numeric_sequence_length)

    # 1つ目の数列生成に必要な数列の長さを測定前処理(トークン列のツリー構造化)
    pr_1 = generate_program.ProgramGenerator(depth)
    pr_1.convert_token_sequence_to_tree(data['token_sequence_1'])

    # 1つ目の数列生成に必要な数列の長さを測定
    necessary_numeric_sequence_length_1 = generate_program.calculate_original_sequence_length(pr_1.root_node, necessary_numeric_sequence_length_2)

    # 1つ目の数列を生成
    x = list(range(necessary_numeric_sequence_length_1))

    calc_1 = program.ProgramStack(data['token_sequence_1'], numeric_sequence_length=necessary_numeric_sequence_length_1)
    calc_stack_1 = calc_1.build()
    seq_1 = calc_stack_1[0].calc(x)
    ...
    data.update(
        {
            ...
            'token_sequence_2': pr_2.get_token_sequence(),
