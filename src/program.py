...
class Compr(Program):
    ...
    def compr_impl(self, f:Callable[[int, int], int], a:int) -> int:
        if a == 0:
            m = 0
            while m < self.max_iter or self.max_iter==-1:
                if f(m, 0) <= 0:
                    return m
                m += 1
...
class ProgramStack:
    ...
    def __init__(self, rpn, max_iter=-1):
        self.rpn = rpn
        self.stack = []
        self.max_iter=max_iter
    ...
    def build(self):
        for s in self.rpn:
            if isinstance(s, int) or s in ['0', '1', '2']:
                ...
                f = self.stack.pop()
                compr=Compr(f=f, a=a)
                compr.update_max_iter(self.max_iter)
                self.stack.append(compr)
            elif s == 'plus':
                b = self.stack.pop()
                a = self.stack.pop()
